print.fecrt_results <- function(x=list(), filename=FALSE, ...){
...
olderprint.fecrt.results <- function(x=list(), filename=FALSE, ...){
	...
	if(is.na(x$dshapequant[1])) cat("The varability was fixed\n\n") else cat(paste("The median and ", x$confidence, "% credible intervals for the change in shape parameter (<1 denotes an increase in variability and >1 a decrease in variability):\nl", x$confidence, ": ", round(x$dshapequant[1],digits+1), "    median: ", round(x$dshapequant[2],digits+1), "    u", x$confidence, ": ", round(x$dshapequant[3],digits+1), "\n\n", sep=""), file=file,append=TRUE)
